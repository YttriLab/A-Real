...
def handle_missing_bp(animal_skeletons: list, all_pose):
    ...
    # print(DeepLabStream._data_output[camera])
    # try:
    # # all_pose.append(animal_skeletons[0])
    #     print(len(all_pose), all_pose)
    # # if len(all_pose) > 1000:
    # #     all_pose
    # except:
    #     pass
    #     all_pose = []
        ...
        for bodypart, coordinates in skeleton.items():
            ...
            if any(np.isnan(np_coords)):
                if HANDLE_MISSING == "pass":
                    ...
                    # previous skeleton
                    # try:
                    #     print(all_pose[-2][0][bodypart])
                    #     print(50*'=')
                    #     print(animal_skeletons[0][bodypart])
                    # except:
                    #     pass
                    try:
                        animal_skeletons[0][bodypart] = all_pose[-2][0][bodypart]
                    except:
                        pass
...
def calculate_skeletons(peaks: dict, animals_number: int, all_pose) -> list:
    """
    Creating skeletons from given peaks
    There could be no more skeletons than animals_number
    Only unique skeletons output
    ...
    animal_skeletons = handle_missing_bp(animal_skeletons, all_pose)
