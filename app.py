...
from utils.configloader import MULTI_CAM, STREAMS, RECORD_EXP, TIME_STAMP
...
class ButtonWindow(QWidget):
    ...
    def start_experiment(self):
        ...
        exp_start_time = datetime.now().strftime('%Y%m%d_%H_%M_%S')
        exact_exp_start_time = time.time()
        # preamble = fr'D:\time_stamps\{exp_start_time}.csv'

        import configparser as cfg
        dsc_config = cfg.ConfigParser()
        dsc_config.read(r'D:\DeepLabStream\settings.ini')
        preamble = dsc_config['Streaming']['TIME_STAMP_DIRECTORY']
        dsc_config['Streaming']['TIME_STAMP_FILE'] = str.join('', (preamble, exp_start_time, '.csv'))
        with open (r'D:\DeepLabStream\settings.ini', 'w') as configfile:
            dsc_config.write(configfile)

        # exact_exp_start_time = datetime.now().strftime('%Y%m%d_%H:%M:%S:%f')

        with open(str.join('', (preamble, exp_start_time, '.csv')), 'w') as csvfile:
            filewriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            filewriter.writerow(['Experiment_Start'])
            filewriter.writerow([exact_exp_start_time])
        if RECORD_EXP:
