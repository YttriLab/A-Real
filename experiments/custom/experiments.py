...
from experiments.custom.ledtrigger import *
...
class BsoidBehaviorPoolExperiment:
    ...
    def __init__(self):
        ...
        self._trial_count = {trial: 0 for trial in self._trials}
        self._trial_timers = {trial: Timer(0) for trial in self._trials}
        # self._trial_timers = {trial: Timer(1) for trial in self._trials} ## Rabira/Hsu edit 070221
        self._exp_timer = Timer(20000000)
    ...
    def check_skeleton(self, frame, skeleton):
        ...
        for trial in self._trial_count:
            # checking if any trial hit a predefined cap
            if self._trial_count[trial] >= 10000000:
                self.stop_experiment()
        ...
        if not self.experiment_finished:
            for trial in self._trials:
                ...
                if result:
                    if self._current_trial is None:
                        if not self._trial_timers[trial].check_timer():
                            # matched_time = datetime.now().strftime('%Y%m%d_%H:%M:%S:%f')
                            self._current_trial = trial
                            self._trial_timers[trial].reset()
                            self._trial_count[trial] += 1
                            # print(trial, self._trial_count[trial])
                            ...
                            timestamp_csv_file = dsc_config['Streaming']['TIME_STAMP_FILE']

                            # print(timestamp_csv_file)
                            matched_time = time.time()
                            print(matched_time)
                            # led.LEDon()
                            df_timestamp = pd.read_csv(timestamp_csv_file, low_memory=False)

                            df_timestamp[self._trial_count[trial], 'Stim_Start'] = matched_time
                            df_timestamp.to_csv(timestamp_csv_file, index=False)
                else:
                    # led.LEDoff()
                    if self._current_trial == trial:
                        self._current_trial = None
