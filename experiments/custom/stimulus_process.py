...
from experiments.custom.ledtrigger import LEDstim
import inspect
...
import random
from utils.configloader import TIME_STAMP

# class Experiment_ST():
#     def __init__(self):
#         self.start_time = 'start_time'
#
#     def load_start_time(self, experiment_start_time):
#         self.start_time = experiment_start_time
#
...
def example_protocol_run(condition_q: mp.Queue):
    ...
    while True:
        ...
        if current_trial is not None:
            # grabs current time of event
            current_time = time.time()

            exact_exp_time = datetime.now().strftime('%H:%M:%S:%f')
            #start_sec = int(exact_exp_time.split(':')[1])*60000000 + (exact_exp_time.split(':')[2])*1000000 + int(exact_exp_time.split(':')[3])

            exact_exp_endtime = datetime.now().strftime('%H:%M:%S:%f')
            #end_sec = int(exact_exp_endtime.split(':')[1])*60000000 + (exact_exp_endtime.split(':')[2])*1000000 + int(exact_exp_endtime.split(':')[3])
            #print(exact_exp_time)
            #print(exact_exp_endtime)

            if stim.off and start_time is None:
                stim.LEDon()
                # print('first time')
            ...
            elif stim.off and (current_time-start_time) > 0:
                stim.LEDon()
            ...
            # print('stim toggled')
            # stim.toggle()
            # with open(TIME_STAMP, 'a+', newline='') as csvfile:
            #     writer = csv.writer(csvfile, delimiter=',')
            #     writer.writerow(['N', exact_exp_time, exact_exp_endtime])
            # # dmod_device.toggle()
            # # led_machine.turn_on()
            # exact_exp_time = datetime.now().strftime('%H:%M:%S:%f')
            # #start_sec = int(exact_exp_time.split(':')[1])*60000000 + (exact_exp_time.split(':')[2])*1000000 + int(exact_exp_time.split(':')[3])
